...
def simulate_fiber_cut(state, level = 1):
    ...
    if level == 1:
        if state == "raised":
            ...
            for onu in topology.onus:
                splv2s.append(onu.splitter2)

            splv2s = list(set(splv2s))
            number_splitter_lv2 = random.randint(0, 3)
            cut_splitters = random.sample(splv2s, number_splitter_lv2)

            splv1s = []
            for onu in topology.onus:
                if onu.splitter2 in cut_splitters:
                    continue
            ...
            number_splitter_lv1 = random.randint(0, 3 - number_splitter_lv2)
            if number_splitter_lv1 == 0 and number_splitter_lv2 == 0:
                number_splitter_lv1 = random.randint(1, 3)

            cut_splitters.extend(random.sample(splv1s, number_splitter_lv1))
            d = {
    ...
    for cut_splitter in cut_splitters:
        for onu in topology.onus:
            if onu.splitter1 == cut_splitter or onu.splitter2 == cut_splitter:
